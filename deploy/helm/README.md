...
Deploys the whole application to OpenShift:

- **Global services** (singletons, shared by every distribution center):
  `po-ingest-api`, `supervisor-api`.
- **Distribution centers** (`values.yaml: distributionCenters`): each entry
  is a self-contained "local" grouping — `local-dc-agent`, `local-wms-api`,
  `local-inventory-robot-api`, `local-shipping-api` — with 1 replica each.
  The `local-*` components in a distribution center are wired to talk only
  to each other (plus the two global services) via in-cluster Service DNS;
  they never reach across into another distribution center's grouping.
  Ships with one entry, "Distribution Center A". Add more entries to stand
  up additional, fully isolated distribution centers under the same
  release.

Per-service inventory/shelf data is seeded from ConfigMaps
(`wmsApi.inventoryCsv`, `robotApi.shelvesCsv` in `values.yaml`), mounted
over each container's `data/*.csv` path — override these per distribution
center to give each one its own starting inventory.
...
pass it directly (`--set openai.apiKey=...`, dev/test only) or point at a
secret you create yourself:

```sh
oc create secret generic my-openai-secret --from-literal=OPENAI_API_KEY=sk-...
helm install adc deploy/helm --set openai.existingSecret=my-openai-secret
```

## Adding a distribution center

Copy the `distributionCenters[0]` block in `values.yaml`, give it a new
`name` (used to derive Service/ConfigMap names — keep it a short,
...
- `route.openshift.io/v1` Routes are created for `po-ingest-api` (PO
  ...
  endpoint) by default; toggle via `openshift.routes.enabled` and each
  component's `route.enabled`. Set `openshift.routes.enabled=false` to
  deploy to plain Kubernetes instead.

## Useful commands

```sh
helm lint deploy/helm
helm template adc deploy/helm --set openai.apiKey=sk-test
